...
function SessionCard({ session }: { session: (typeof MOCK_PAST_SESSIONS)[0] }) {
  ...
  return (
    ...
    >
      ...
      <Text style={[styles.sessionDay, { color: colors.mutedForeground }]}>
        {session.dayName}
      </Text>
        ...
        <View style={styles.section}>
          ...
          >
            <View style={styles.workoutTop}>
              <View>
                <Text style={[styles.workoutDay, { color: colors.primary }]}>
                  {todayDay.dayName}
                </Text>
                <Text style={[styles.workoutFocus, { color: colors.foreground }]}>
                  {todayDay.focus}
                </Text>
                <Text style={[styles.workoutPlan, { color: colors.mutedForeground }]}>
                  {MOCK_PLAN.name}
                </Text>
              </View>
              <View style={styles.workoutMeta}>
                <Text style={[styles.exerciseCount, { color: colors.accent }]}>
                  {todayDay.exercises.length}
                </Text>
                <Text style={[styles.exerciseLabel, { color: colors.mutedForeground }]}>
                  exercises
                </Text>
              </View>
            </View>
            ...
            <View style={styles.muscleRow}>
              {[...new Set(todayDay.exercises.map((e) => e.exercise.muscleGroup))].map((mg) => (
                <View
                  key={mg}
            ...
            <Animated.View style={{ transform: [{ scale }] }}>
              <Pressable
                ...
                onPress={async () => {
                  ...
                  router.push(`/workout?dayId=${todayDay.id}`);
                }}
