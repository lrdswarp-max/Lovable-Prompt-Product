...
import { api, type ApiMessage, type ApiConversation } from "@/lib/api";
...
export default function StudentChatScreen() {
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      async function loadConversations() {
        try {
          const convs = await api.conversations.list(user!.id);
          if (!active) return;
          setConversations(convs);
          // Default to first 1:1 conversation
          const firstConv = convs.find((c) => !c.isGroup) ?? convs[0];
          if (firstConv && !convId) {
            setConvId(firstConv.id);
          }
        } catch {
          // keep empty
        } finally {
  ...
  const trainerName = activeConv?.participantNames.find((n) => n !== user?.name) ?? "Coach";
  ...
  const handleSend = async () => {
    ...
    setInput("");
    try {
      const msg = await api.conversations.sendMessage(convId, {
        senderId: user.id,
      ...
      // Optimistic update failed silently
    }
  };
      ...
      <View style={[styles.bubbleRow, isMe ? styles.bubbleRowRight : styles.bubbleRowLeft]}>
        {!isMe && (
          <View style={[styles.avatar, { backgroundColor: colors.primary + "30" }]}>
            <Text style={[styles.avatarText, { color: colors.primary }]}>{item.senderName.charAt(0)}</Text>
          </View>
        )}
        <View
          style={[
  ...
  return (
    ...
    >
      <View style={[styles.chatHeader, { paddingTop: topPad + 12, backgroundColor: colors.background, borderBottomColor: colors.border }]}>
        <View style={[styles.avatarLg, { backgroundColor: colors.primary + "30" }]}>
          <Text style={[styles.avatarLgText, { color: colors.primary }]}>{trainerName.charAt(0)}</Text>
        </View>
        <View>
...
const styles = StyleSheet.create({
  root: { flex: 1 },
  chatHeader: { flexDirection: "row", alignItems: "center", gap: 12, paddingHorizontal: 20, paddingBottom: 14, borderBottomWidth: 1 },
